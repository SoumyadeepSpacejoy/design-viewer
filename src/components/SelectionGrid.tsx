...
const features: FeatureItem[] = [
  ...
  },
  {
    ...
    description: "Explore neural generated interior architectures and inspirations.",
    roles: ["admin", "owner"],
    icon: () => (
      <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M12 2v8" />
    ...
    <Show
      ...
      fallback={
        <div class="flex flex-col items-center justify-center min-h-[400px]">
          ...
          <p class="text-muted-foreground text-[10px] font-bold uppercase tracking-widest mt-6">
    ...
    >
      <div class="max-w-6xl mx-auto px-4">
        <div class="text-center mb-16 sm:mb-24">
          <h1 class="text-4xl sm:text-6xl font-medium tracking-tight mb-6 text-foreground">
            Precision <span class="text-primary font-bold">Intelligence</span>
          </h1>
          <p class="text-muted-foreground font-medium max-w-xl mx-auto text-sm sm:text-base leading-relaxed">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8 stagger-items">
          <For each={filteredFeatures()}>
            {(feature) => (
              <div
                class="premium-card group cursor-pointer p-8 sm:p-10 flex flex-col gap-6"
                onClick={() => props.onSelect(feature.id)}
              >
                <div class="w-16 h-16 rounded-2xl bg-primary/10 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-primary-foreground transition-all duration-500 shadow-inner">
                  {feature.icon()}
                </div>

                <div class="space-y-3">
                  <h3 class="text-2xl font-bold tracking-tight group-hover:text-primary transition-colors text-foreground">
                    {feature.title}
                  </h3>
                  <p class="text-muted-foreground/90 font-medium text-sm leading-relaxed line-clamp-2">
                    {feature.description}
                  </p>
                </div>

                <div class="mt-auto pt-6 flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-primary opacity-0 group-hover:opacity-100 transform translate-x-[-10px] group-hover:translate-x-0 transition-all duration-500">
                  Initialize Module
